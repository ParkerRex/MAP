...
struct SessionsCompactParams {
    ...
    session_key: Option<String>,
    key: Option<String>,
    #[serde(alias = "keep_messages")]
    keep_messages: Option<i64>,
    #[serde(alias = "keep_runs")]
...
fn clamp_i64(value: Option<i64>, default: i64, min: i64, max: i64) -> i64 {
...
#[derive(Debug, FromRow)]
struct SessionPreviewSummaryRow {
    id: Uuid,
    title: Option<String>,
    session_key: Option<String>,
...
#[derive(Debug, FromRow)]
struct SessionUsageAggregateRow {
    message_count: i64,
    ...
    run_done_count: i64,
    run_error_count: i64,
    run_aborted_count: i64,
    run_needs_confirmation_count: i64,
    prompt_char_count: i64,
    output_char_count: i64,
    last_message_at: Option<chrono::DateTime<Utc>>,
    last_run_at: Option<chrono::DateTime<Utc>>,
}

#[derive(Debug, FromRow)]
struct SessionUsageLogRow {
    created_at: chrono::DateTime<Utc>,
    event_id: String,
    event_kind: String,
    payload: Value,
...
#[derive(Debug, FromRow)]
struct SessionUsageTimeseriesRow {
    bucket_at: chrono::DateTime<Utc>,
    run_count: i64,
    run_done_count: i64,
    run_error_count: i64,
    run_aborted_count: i64,
...
async fn dispatch_request(
    ...
    if routes::ws_methods::is_unavailable_stub_method(method) {
        let message =
            format!("`{method}` is recognized but not implemented in MAP Rust gateway yet");
        return send_error(socket, &request.id, "unavailable", &message).await;
    }
    ...
    match method {
        ...
        "sessions.preview" => {
            let params =
                match serde_json::from_value::<SessionsPreviewParams>(request.params.clone()) {
                    Ok(params) => params,
                    Err(_) => {
                        return send_error(
                            socket,
                            &request.id,
                            "invalid_request",
                            "invalid sessions.preview params",
                        )
                        .await;
                    }
                };
            ...
            let session_id = params.session_id.or(params.id);
            let session_key = params.session_key.or(params.key).and_then(|value| {
            ...
            if session_id.is_some() || session_key.is_some() {
                let session = match resolve_existing_session_with_refs(
                    state,
                    session_id,
                    session_key,
                )
                ...
                let message_limit = clamp_i64(params.message_limit, 8, 1, 200);
                let run_limit = clamp_i64(params.run_limit, 8, 1, 200);
                ...
                let run_count = sqlx::query_scalar::<_, i64>(
                    "select count(*) from chat_runs where session_id = $1",
                )
                .bind(session.id)
                .fetch_one(&state.pool)
                .await
                .map_err(|_| ())?;
                let last_message_at = sqlx::query_scalar::<_, Option<chrono::DateTime<Utc>>>(
                ...
                .await
                .map_err(|_| ())?;
                let last_run_at = sqlx::query_scalar::<_, Option<chrono::DateTime<Utc>>>(
                    "select max(created_at) from chat_runs where session_id = $1",
                )
                .bind(session.id)
                .fetch_one(&state.pool)
                .await
                ...
                return send_ok(
                    ...
                    json!({
                        ...
                        "latest": {
                            "messageAt": last_message_at,
                            "runAt": last_run_at
                        },
                        "messages": messages
                            .into_iter()
                            .map(|row| {
                                let text_preview = row.text.chars().take(280).collect::<String>();
                                json!({
                                    "id": row.id,
                                    "role": row.role,
                        ...
                        "runs": runs
                            .into_iter()
                            .map(|row| {
                                let prompt_preview = row.prompt.chars().take(180).collect::<String>();
                                let output_preview = row.output.chars().take(280).collect::<String>();
                                json!({
                                    "id": row.id,
                                    "status": row.status,
            ...
            let limit = clamp_i64(params.limit, 25, 1, 200);
            let rows = sqlx::query_as::<_, SessionPreviewSummaryRow>(
                r#"
                select
                  s.id,
                  s.title,
                  ...
                  coalesce(cr.run_count, 0)::bigint as run_count,
                  sm.last_message_at,
                  cr.last_run_at
                from sessions s
                left join (
        ...
        "sessions.compact" => {
            let params =
                match serde_json::from_value::<SessionsCompactParams>(request.params.clone()) {
                    Ok(params) => params,
                    Err(_) => {
                        return send_error(
                            socket,
                            &request.id,
                            "invalid_request",
                            "invalid sessions.compact params",
                        )
                        .await;
                    }
                };
            ...
            if session_id.is_none() && session_key.is_none() {
                return send_error(
                    ...
                    &request.id,
                    "invalid_request",
                    "sessionId or sessionKey is required",
                )
                .await;
            }

            let session =
                match resolve_existing_session_with_refs(state, session_id, session_key).await {
                    Ok(session) => session,
                    Err(error) => return send_api_error(socket, &request.id, error).await,
                };
            let keep_messages = clamp_i64(params.keep_messages, 250, 0, 5_000);
            let keep_runs = clamp_i64(params.keep_runs, 100, 0, 5_000);
            let dry_run = params.dry_run.unwrap_or(false);

            let before_messages = sqlx::query_scalar::<_, i64>(
                "select count(*) from session_messages where session_id = $1",
            )
            .bind(session.id)
            .fetch_one(&state.pool)
            .await
            .map_err(|_| ())?;
            let before_runs = sqlx::query_scalar::<_, i64>(
                "select count(*) from chat_runs where session_id = $1",
            )
            .bind(session.id)
            .fetch_one(&state.pool)
            ...
            let stale_messages = sqlx::query_scalar::<_, i64>(
                r#"
                select count(*)::bigint
                from (
                  select id
            ...
            .await
            .map_err(|_| ())?;
            let stale_runs = sqlx::query_scalar::<_, i64>(
                r#"
                select count(*)::bigint
                from (
                  select id
            ...
            let (deleted_messages, deleted_runs) = if dry_run {
                (stale_messages, stale_runs)
            } else {
                ...
                .map_err(|_| ())?
                .rows_affected() as i64;
                if deleted_messages > 0 || deleted_runs > 0 {
                    let _ = sqlx::query("update sessions set updated_at = now() where id = $1")
                        .bind(session.id)
                        .execute(&state.pool)
                        .await;
                }
                (deleted_messages, deleted_runs)
            };

            let after_messages = (before_messages - deleted_messages).max(0);
            let after_runs = (before_runs - deleted_runs).max(0);
            send_ok(
                socket,
                &request.id,
                json!({
        ...
        "sessions.delete" => {
            let params =
                match serde_json::from_value::<SessionsDeleteParams>(request.params.clone()) {
                    Ok(params) => params,
                    Err(_) => {
                        return send_error(
                            socket,
                            &request.id,
                            "invalid_request",
                            "invalid sessions.delete params",
                        )
                        .await;
                    }
                };
            ...
            let deleted = if let Some(session_id) = session_id {
                sqlx::query_as::<_, (Uuid,)>("delete from sessions where id = $1 returning id")
                    .bind(session_id)
                    .fetch_optional(&state.pool)
                    .await
                    .map_err(|_| ())?
                ...
                .bind(session_key.unwrap_or_default())
                .fetch_optional(&state.pool)
                .await
                .map_err(|_| ())?
                .map(|row| row.0)
            ...
            let Some(deleted_session_id) = deleted else {
                return send_error(socket, &request.id, "not_found", "session not found").await;
            };
            send_ok(
                socket,
                &request.id,
        ...
        "sessions.usage" => {
            let params = match serde_json::from_value::<SessionsUsageParams>(request.params.clone())
            {
                Ok(params) => params,
                Err(_) => {
                    return send_error(
            ...
            let window_days = clamp_i64(params.window_days, 30, 1, 365);
            let usage = sqlx::query_as::<_, SessionUsageAggregateRow>(
                r#"
                ...
                run_stats as (
                  select
                    count(*)::bigint as run_count,
                    count(*) filter (where status = 'done')::bigint as run_done_count,
                    count(*) filter (where status = 'error')::bigint as run_error_count,
                    count(*) filter (where status = 'aborted')::bigint as run_aborted_count,
                    count(*) filter (where status = 'needs_confirmation')::bigint as run_needs_confirmation_count,
                    coalesce(sum(length(prompt)), 0)::bigint as prompt_char_count,
                    coalesce(sum(length(output)), 0)::bigint as output_char_count,
                    max(created_at) as last_run_at
                  from chat_runs
                  where session_id = $1
                    and created_at >= now() - make_interval(days => $2)
                ...
                select
                  ...
                  message_stats.user_message_count,
                  message_stats.assistant_message_count,
                  run_stats.run_count,
                  run_stats.run_done_count,
                  run_stats.run_error_count,
                  run_stats.run_aborted_count,
                  run_stats.run_needs_confirmation_count,
                  run_stats.prompt_char_count,
                  run_stats.output_char_count,
            ...
            let session_key = lookup_session_key(state, session.id).await.ok().flatten();
            let latest_activity_at = match (usage.last_message_at, usage.last_run_at) {
            ...
            send_ok(
                ...
                json!({
                    ...
                    "messages": {
                        ...
                        "assistant": usage.assistant_message_count,
                    },
                    "runs": {
                        "total": usage.run_count,
                        "done": usage.run_done_count,
                        "error": usage.run_error_count,
                        "aborted": usage.run_aborted_count,
                        "needsConfirmation": usage.run_needs_confirmation_count,
                    },
                    "chars": {
                        "prompt": usage.prompt_char_count,
                        "output": usage.output_char_count,
                    },
        ...
        "sessions.usage.logs" => {
            let params =
                match serde_json::from_value::<SessionsUsageLogsParams>(request.params.clone()) {
                    Ok(params) => params,
                    Err(_) => {
                        return send_error(
                            socket,
                            &request.id,
                            "invalid_request",
                            "invalid sessions.usage.logs params",
                        )
                        .await;
                    }
                };
            ...
            let limit = clamp_i64(params.limit, 50, 1, 200);
            let cursor = match params.cursor {
                Some(value) => match chrono::DateTime::<Utc>::from_timestamp_millis(value) {
                    Some(ts) => Some(ts),
                    None => {
                        return send_error(
                            socket,
                            &request.id,
                            "invalid_request",
                            "cursor must be a unix epoch timestamp in milliseconds",
                        )
                        .await;
                    }
                },
            ...
            let mut rows = sqlx::query_as::<_, SessionUsageLogRow>(
                r#"
                select created_at, event_id, event_kind, payload
                from (
                  select
                    created_at,
                    id::text as event_id,
                    'run'::text as event_kind,
                    jsonb_build_object(
                      'runId', id,
                      'status', status,
                      'modelUsed', metadata ->> 'model_used',
                      'promptPreview', left(prompt, 180),
                      'outputPreview', left(output, 280)
                    ) as payload
                  from chat_runs
                  where session_id = $1
            ...
            let next_cursor = rows.last().map(|row| row.created_at.timestamp_millis());
            let session_key = lookup_session_key(state, session.id).await.ok().flatten();
            let logs = rows
                .into_iter()
                .map(|row| {
                    json!({
                        "cursor": row.created_at.timestamp_millis(),
        ...
        "sessions.usage.timeseries" => {
            let params = match serde_json::from_value::<SessionsUsageTimeseriesParams>(
                ...
                Err(_) => {
                    return send_error(
                        ...
                        &request.id,
                        "invalid_request",
                        "invalid sessions.usage.timeseries params",
                    )
                    .await;
                }
            };
            ...
            let limit = clamp_i64(params.limit, 30, 1, 365);
            let window_days = clamp_i64(params.window_days, 30, 1, 365);
            let mut rows = sqlx::query_as::<_, SessionUsageTimeseriesRow>(
                r#"
                with run_series as (
                  select
                    date_trunc($2, created_at) as bucket_at,
                    count(*)::bigint as run_count,
                    count(*) filter (where status = 'done')::bigint as run_done_count,
                    count(*) filter (where status = 'error')::bigint as run_error_count,
                    count(*) filter (where status = 'aborted')::bigint as run_aborted_count,
                ...
                select
                  coalesce(run_series.bucket_at, message_series.bucket_at) as bucket_at,
                  coalesce(run_series.run_count, 0)::bigint as run_count,
                  coalesce(run_series.run_done_count, 0)::bigint as run_done_count,
                  coalesce(run_series.run_error_count, 0)::bigint as run_error_count,
                  coalesce(run_series.run_aborted_count, 0)::bigint as run_aborted_count,
                  coalesce(run_series.run_needs_confirmation_count, 0)::bigint as run_needs_confirmation_count,
                  coalesce(message_series.message_count, 0)::bigint as message_count,
                  coalesce(message_series.user_message_count, 0)::bigint as user_message_count,
                  coalesce(message_series.assistant_message_count, 0)::bigint as assistant_message_count
                from run_series
                full outer join message_series
                  on run_series.bucket_at = message_series.bucket_at
                    ...
                    "series": rows
                        ...
                        .map(|row| {
                            json!({
                                ...
                                "runs": {
                                    "total": row.run_count,
                                    "done": row.run_done_count,
                                    "error": row.run_error_count,
                                    "aborted": row.run_aborted_count,
                                    "needsConfirmation": row.run_needs_confirmation_count,
                                },
                                "messages": {
                                    "total": row.message_count,
                                    "user": row.user_message_count,
                                    "assistant": row.assistant_message_count,
                                }
                            })
                        })
                        .collect::<Vec<_>>()
        ...
        "usage.status" => {
            let params = serde_json::from_value::<UsageStatusParams>(request.params.clone())
                .unwrap_or_default();
            let window_days = clamp_i64(params.window_days, 30, 1, 365);
            ...
            let total_runs = sqlx::query_scalar::<_, i64>(
                "select count(*) from chat_runs where created_at >= now() - make_interval(days => $1)",
            )
            .bind(window_days)
            .fetch_one(&state.pool)
            .await
            ...
            let total_messages = sqlx::query_scalar::<_, i64>(
                "select count(*) from session_messages where created_at >= now() - make_interval(days => $1)",
            )
            .bind(window_days)
            .fetch_one(&state.pool)
            .await
            .map_err(|_| ())?;
            let runs_with_cost = sqlx::query_scalar::<_, i64>(
                r#"
                select
                  count(*) filter (where metadata ? 'cost_usd' or metadata ? 'costUsd')::bigint
                from chat_runs
                where created_at >= now() - make_interval(days => $1)
                "#,
            )
            ...
            .await
            .map_err(|_| ())?;
            let latest_run_at = sqlx::query_scalar::<_, Option<chrono::DateTime<Utc>>>(
                "select max(created_at) from chat_runs",
            )
            .fetch_one(&state.pool)
            .await
            .map_err(|_| ())?;
            ...
            send_ok(
                ...
                json!({
                    ...
                    "capabilities": {
                        "sessionUsage": true,
                        "sessionUsageLogs": true,
                        "sessionUsageTimeseries": true,
                        "costTracking": runs_with_cost > 0
                    },
                    "notes": if runs_with_cost == 0 {
                        vec!["Cost tracking is unavailable because no run metadata includes cost fields (`cost_usd` / `costUsd`)."]
                    } else {
                        Vec::<&str>::new()
                    }
                }),
        ...
        "usage.cost" => {
            let params = match serde_json::from_value::<UsageCostParams>(request.params.clone()) {
                Ok(params) => params,
                Err(_) => {
                    return send_error(
                        socket,
                        &request.id,
                        "invalid_request",
                        "invalid usage.cost params",
                    )
                    .await;
                }
            };
            let window_days = clamp_i64(params.window_days, 30, 1, 365);
            let (run_count, runs_with_cost) = sqlx::query_as::<_, (i64, i64)>(
                r#"
                select
                  count(*)::bigint as run_count,
                  count(*) filter (where metadata ? 'cost_usd' or metadata ? 'costUsd')::bigint as runs_with_cost
                from chat_runs
                where created_at >= now() - make_interval(days => $1)
                "#,
            )
            ...
            if runs_with_cost == 0 {
                return send_error(
                    ...
                    &request.id,
                    "unavailable",
                    "usage.cost is unavailable: no run metadata includes `cost_usd` or `costUsd`.",
                )
                .await;
            }

            let total_cost_usd = sqlx::query_scalar::<_, f64>(
                r#"
                select
                  coalesce(sum(
                    case
                      when metadata ? 'cost_usd'
                        and trim(metadata ->> 'cost_usd') ~ '^-?\d+(\.\d+)?$'
                        then trim(metadata ->> 'cost_usd')::double precision
                      when metadata ? 'costUsd'
                        and trim(metadata ->> 'costUsd') ~ '^-?\d+(\.\d+)?$'
                        then trim(metadata ->> 'costUsd')::double precision
                      else null
                    end
...
async fn resolve_existing_session_with_refs(
    state: &AppState,
    session_id: Option<Uuid>,
    session_key: Option<String>,
) -> Result<SessionRow, ApiError> {
    ...
    let Some(key) = normalized_key else {
        return Err(ApiError::BadRequest(
            "sessionId or sessionKey is required".to_string(),
        ));
    };
...
mod tests {
    ...
    #[test]
    fn stream_five_methods_are_not_unavailable_stubs() {
        ...
        for method in implemented_methods {
            assert!(
                ...
                "`{method}` should not be marked as unavailable after stream 5 implementation"
            );
        }
    }

    #[test]
    fn stream_two_methods_are_not_unavailable_stubs() {
        let implemented_methods = [
            "sessions.compact",
            "sessions.delete",
            "sessions.preview",
        ...
        for method in implemented_methods {
            assert!(
                !routes::ws_methods::is_unavailable_stub_method(method),
                "`{method}` should not be marked as unavailable after stream 2 implementation"
            );
        }
    }

    #[test]
    fn stream_four_methods_are_not_unavailable_stubs() {
        let implemented_methods = ["cron.status", "cron.update", "talk.mode", "update.run"];

        for method in implemented_methods {
            assert!(
                !routes::ws_methods::is_unavailable_stub_method(method),
                "`{method}` should not be marked as unavailable after stream 4 implementation"
            );
        }
    }
    ...
    fn ws_param_aliases_accept_map_and_openclaw_control_plane_shapes() {
        ...
        let map_sessions_compact = serde_json::from_value::<SessionsCompactParams>(json!({
            "sessionId": session_id,
            "keepMessages": 25,
            "keepRuns": 8,
            "dryRun": true
        }))
        .expect("map-style sessions.compact params should parse");
        assert_eq!(map_sessions_compact.session_id, Some(session_id));
        assert_eq!(map_sessions_compact.keep_messages, Some(25));
        assert_eq!(map_sessions_compact.keep_runs, Some(8));
        assert_eq!(map_sessions_compact.dry_run, Some(true));

        let openclaw_sessions_delete = serde_json::from_value::<SessionsDeleteParams>(json!({
            "session_key": "main"
        }))
        .expect("openclaw-style sessions.delete params should parse");
        assert_eq!(
            openclaw_sessions_delete.session_key.as_deref(),
            Some("main")
        );

        let map_sessions_usage_logs = serde_json::from_value::<SessionsUsageLogsParams>(json!({
            "sessionId": session_id,
            "cursor": 1_725_000_000_000_i64,
            "limit": 250
        }))
        .expect("map-style sessions.usage.logs params should parse");
        assert_eq!(map_sessions_usage_logs.session_id, Some(session_id));
        assert_eq!(map_sessions_usage_logs.cursor, Some(1_725_000_000_000));
        assert_eq!(map_sessions_usage_logs.limit, Some(250));

        let openclaw_sessions_usage_timeseries =
            serde_json::from_value::<SessionsUsageTimeseriesParams>(json!({
                "session_id": session_id,
                "bucket": "hour",
                "window_days": 7
            }))
            .expect("openclaw-style sessions.usage.timeseries params should parse");
        assert_eq!(
            openclaw_sessions_usage_timeseries.session_id,
            Some(session_id)
        );
        assert_eq!(
            openclaw_sessions_usage_timeseries.bucket.as_deref(),
            Some("hour")
        );
        assert_eq!(openclaw_sessions_usage_timeseries.window_days, Some(7));

        let map_usage_cost = serde_json::from_value::<UsageCostParams>(json!({
            "windowDays": 14
        }))
        .expect("map-style usage.cost params should parse");
        assert_eq!(map_usage_cost.window_days, Some(14));

        let openclaw_usage_status = serde_json::from_value::<UsageStatusParams>(json!({
            "window_days": 21
        }))
        .expect("openclaw-style usage.status params should parse");
        assert_eq!(openclaw_usage_status.window_days, Some(21));
    }

    #[test]
    fn stream_two_limit_clamps_enforce_bounds() {
        assert_eq!(clamp_i64(Some(0), 30, 1, 365), 1);
        assert_eq!(clamp_i64(Some(999), 30, 1, 365), 365);
        assert_eq!(clamp_i64(None, 30, 1, 365), 30);

        assert_eq!(clamp_i64(Some(-3), 50, 1, 200), 1);
        assert_eq!(clamp_i64(Some(500), 50, 1, 200), 200);
        assert_eq!(clamp_i64(Some(120), 50, 1, 200), 120);
    }
}
