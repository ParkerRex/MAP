...
END;
$$;
CREATE OR REPLACE FUNCTION "public"."sync_calendars"(user_id UUID, calendars JSONB) RETURNS JSONB AS $$
DECLARE calendar JSONB;
calendars_synced INT := 0;
BEGIN FOR calendar IN
SELECT *
FROM jsonb_array_elements(calendars) LOOP
...
VALUES (
  ...
  ) ON CONFLICT (google_calendar_id, user_id) DO
UPDATE
...
BEGIN FOR event IN
SELECT *
FROM jsonb_array_elements(events) LOOP
...
VALUES (
    ...
    event->>'etag'
  ) ON CONFLICT (google_event_id, user_id) DO
UPDATE
...
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION "public"."update_modified_column"() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION "public"."update_task_tags"(task_id UUID, tag_titles TEXT []) RETURNS VOID AS $$
DECLARE tag_id UUID;
BEGIN
...
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION create_and_start_sync_job(p_user_id UUID, p_job_type TEXT) RETURNS TABLE (
    id UUID,
    user_id UUID,
    status TEXT,
    job_type TEXT,
    ...
    calendars_synced INTEGER,
    events_synced INTEGER
  ) AS $$ BEGIN RETURN QUERY WITH updated_job AS (
    UPDATE sync_job
    SET updated_at = NOW()
...
$$ LANGUAGE plpgsql;
-- Create triggers
CREATE TRIGGER "goal_updated_at" BEFORE
UPDATE ON "public"."goal" FOR EACH ROW EXECUTE FUNCTION "public"."update_modified_column"();
CREATE TRIGGER "update_calendar_event_modtime" BEFORE
UPDATE ON "public"."calendar_event" FOR EACH ROW EXECUTE FUNCTION "public"."update_modified_column"();
CREATE TRIGGER "update_calendar_modtime" BEFORE
UPDATE ON "public"."calendar" FOR EACH ROW EXECUTE FUNCTION "public"."update_modified_column"();
CREATE TRIGGER "update_integration_modtime" BEFORE
UPDATE ON "public"."integration" FOR EACH ROW EXECUTE FUNCTION "public"."update_modified_column"();
...
CREATE POLICY "Allow authenticated user to view & modify their own data" ON "public"."goal" TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Allow authenticated user to view & modify their own data" ON "public"."header" TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Allow authenticated user to view & modify their own data" ON "public"."project" TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can view their own integrations" ON "public"."integration" FOR
SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own integrations" ON "public"."integration" FOR
INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own integrations" ON "public"."integration" FOR
UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their own integrations" ON "public"."integration" FOR DELETE USING (auth.uid() = user_id);
CREATE POLICY "Users can view their own calendar sync info" ON public.calendar_sync_info FOR
SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own calendar sync info" ON public.calendar_sync_info FOR
INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own calendar sync info" ON public.calendar_sync_info FOR
UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their own calendar sync info" ON public.calendar_sync_info FOR DELETE USING (auth.uid() = user_id);
CREATE POLICY "Users can view their own sync jobs" ON public.sync_job FOR
SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own sync jobs" ON public.sync_job FOR
INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own sync jobs" ON public.sync_job FOR
UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can manage their own webhook channels" ON public.webhook_channel FOR ALL USING (auth.uid() = user_id);
