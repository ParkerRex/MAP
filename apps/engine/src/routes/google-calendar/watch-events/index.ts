import { GeneralErrorSchema } from "@/common/schema";
import { OpenAPIHono, createRoute } from "@hono/zod-openapi";
import { z } from "zod";
import { GoogleCalendarProvider } from "../../../providers/google-calendar/google-calendar-provider";
import type { Bindings } from "../../../types";
import { ChannelSchema, WatchRequestSchema } from "./schema";

const app = new OpenAPIHono<{ Bindings: Bindings }>();

const watchEventsRoute = createRoute({
  method: "post",
  path: "/{calendarId}/events/watch",
  summary: "Watch events",
  description: "Creates a new watch for events on a calendar",
  request: {
    params: z.object({
      calendarId: z.string().openapi({ example: "primary" }),
    }),
    body: {
      content: {
        "application/json": {
          schema: WatchRequestSchema,
        },
      },
    },
  },
  responses: {
    200: {
      content: {
        "application/json": {
          schema: ChannelSchema,
        },
      },
      description: "Successfully created watch for events",
    },
    400: {
      content: {
        "application/json": {
          schema: GeneralErrorSchema,
        },
      },
      description: "Bad Request",
    },
  },
});

app.openapi(watchEventsRoute, async (c) => {
  try {
    const { calendarId } = c.req.valid("param");
    const requestBody = c.req.valid("json");
    const provider = new GoogleCalendarProvider(c);
    const data = await provider.watchEvents(calendarId, requestBody);
    return c.json({ data }, 200);
  } catch (error) {
    return c.json(
      {
        error: "Failed to create watch for events",
        message: error instanceof Error ? error.message : "Unknown error",
        requestId: c.get("requestId"),
        code: "400",
      },
      400,
    );
  }
});

export default app;
